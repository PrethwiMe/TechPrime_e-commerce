...
exports.searchProduct = async (req, res) => {
  try {
    let value = req.query.searchKey?.trim() || "";
    ...
    return res.render('user-pages/allProductsUsers.ejs', {
...
exports.sortAndSearchProducts = async (req, res) => {
  try {
    const { sort, searchKey, minPrice, maxPrice, categories } = req.query;
    let query = { isActive: true };
    ...
    if (categories) {
      let cate = categories.split(",").map(c => c.trim()).filter(c => c !== "");
      if (cate.length > 0) {
        query.catagoriesId = { $in: cate };
      }
    }
